...
// of the efforts and automatically getting a lot of the setup and things like authentication handled.
//
...
func (api *API) runHTTPServer(ctx context.Context, l net.Listener) {
	...
	api.config.Logger.Infof("REST API (HTTP): %s", maddr)
...
func (api *API) runLibp2pServer(ctx context.Context) {
	...
	api.config.Logger.Infof("REST API (libp2p-http): ENABLED. Listening on:\n%s\n", listenMsg)
