...
func (rpcapi *RPCAPI) PinPath(ctx context.Context, in []interface{}, out *cid.Cid) error {
	path, pin := in[0].(string), in[1].(api.PinSerial)
	ci, err := rpcapi.c.PinPath(path, pin.ToPin())
	*out = ci
	return err
}
