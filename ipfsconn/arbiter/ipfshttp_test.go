...
func TestPin(t *testing.T) {
	...
	if pinSt.IsPinned(-1) {
		t.Error("cid should not have been pinned")
	}

	pin2 := api.PinCid(test.ErrorCid)
	err = ipfs.Pin(ctx, pin2)
	if err != nil {
		t.Error("errorcid should not reach mock api")
	}
}
...
func TestIPFSUnpin(t *testing.T) {
	...
	pinSt, err := ipfs.PinLsCid(ctx, api.PinCid(c))
	if err != nil {
		t.Fatal("expected success doing ls:", err)
	}
	if !pinSt.IsPinned(-1) {
		t.Error("cid should not have been unpinned")
	}
}
