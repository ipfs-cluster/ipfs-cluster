...
// State retrieves the current consensus State.
func (mc *mockConsensus) State(context.Context) (state.ReadOnly, error) {
	return mc.st, nil
}
...
func testSlowStatelessPinTracker(t *testing.T) *Tracker {
	...
	mpt := New(cfg, test.PeerID1, test.PeerName1, &mockConsensus{st: mockState{slow: true}})
...
func testStatelessPinTracker(t testing.TB) *Tracker {
	...
	cfg.Default()
	cfg.ConcurrentPins = 1
	spt := New(cfg, test.PeerID1, test.PeerName1, &mockConsensus{st: mockState{}})
